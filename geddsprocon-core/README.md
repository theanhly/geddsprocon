...
`withBufferConnectorString(String connectorString)` **(Deprecated)** | String | This sets the connection string to the message-buffer process. If the DSP application restarts due to failure the connection string determines the message-buffer. It is highly recommended to set a buffer string in case of failure. If this setting is omitted an internal unused connection string is generated. **(Deprecated)**
`withRequestAddress(String adrress, int port, String connectorType)` | `router IP string`, `router port`, `["PRIMARY", "SECONDARY"]`  | Only valid for input operators. Determines the IP of the output cluster from which the data is pulled. The `connectorType`determines if it is a primary connection or a secondary connection.
`withRouterAddress(String host, int port)` | `router IP string`, `router port`  | This method is used to set the `DSPRouter`'s address. A `DSPRequester` can then send requests to that address and pull the data.
`withInputOperatorFaultTolerance(boolean faultTolerance)` | `true`, `false` | This value sets up the mode in which the input operators should run on. With _input operator fault tolerance_ turned on, the input operator can access its buffer again after a failure.